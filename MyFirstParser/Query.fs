...
let identifier = many1Chars (letter <|> digit) |>> Expr.Identifer .>> ws
...
opp.AddOperator <| InfixOperator("<=", ws, 12, Associativity.None, fun x y -> Expr.Binary (x, y, BinaryExprKind.GreaterThan))
...
let execute query source =
  let rec evaluate expr element =
    ...
    | Identifer n -> element.GetType().GetProperty(n).GetValue(element)
